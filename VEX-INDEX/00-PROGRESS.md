...
- [x] ✅ OpenRouter ↔ vex-app — F4 FIXED (commit `e02f73b`): `loadConfig` memoized on the provider instance (1h TTL + 30s stale-retry throttle, serve-last-good on transient `/models` failure, model-absent still null, in-flight dedup + per-caller copy). "No resetProvider on unlock" is moot (boot null is not cached + lock reset). Streaming unchanged.
...
- 2026-05-29: BUNDLE B — F4 SHIPPED (OpenRouter `loadConfig` `/models` cache; `/codex`-gated, committed
  `e02f73b`, push pending user OK). `OpenRouterProvider.loadConfig()` (`src/vex-agent/inference/openrouter.ts`)
  fetched the full `/models` catalog on EVERY turn (6 registry-singleton call sites) just to read the configured
  model's pricing — a per-turn network round-trip that also amplified failures (a transient `/models` blip
  returned null and killed an otherwise-healthy turn with "No inference config available"). loadConfig now
  memoizes the successful `InferenceConfig` on the provider instance: fresh hit within `MODEL_CONFIG_CACHE_TTL_MS`
  (1h); concurrent callers dedup on ONE in-flight fetch (resolves to the canonical ref — every caller returns its
  own shallow copy, so the cache can't be mutated); a transient metadata failure WITH a last-good serves stale +
  throttles the next attempt (`MODEL_CONFIG_STALE_RETRY_MS` 30s) so an outage no longer blocks every turn; a
  SUCCESSFUL catalog missing the model (delist/misconfig) still returns null (loud); first-fetch with no last-good
  → null (unchanged). The raw `/models` read moved verbatim into a private `_fetchConfig()` returning a
  discriminated `success | model_not_found | metadata_unavailable`. The 6 call sites + `loadConfig` signature
  UNCHANGED; the chunker's per-job `new OpenRouterProvider()` (`compact-jobs/chunker-call.ts`) intentionally out
  of scope; `types.ts` contract docstring corrected (was "called once at startup").
  Process: inline recon (loadConfig path + provider lifecycle + chunker exception) → Codex named session
  `f4-harness` (design RED [serve-stale must not mask model_not_found; add stale-retry throttle; don't hand out
  mutable config identity] → RED [in-flight returned a shared object reference to all awaiters] → GREEN; final
  review on the live diff GREEN after one doc-phrase fix). Tests: new `openrouter-loadconfig.test.ts` (8 cases:
  TTL reuse, concurrent dedup + copy isolation, cold-null, stale-serve, throttle-then-reattempt, model_not_found
  null, pricing refresh, copy-guard); 14 with registry; inference + engine-runner consumers 160 pass; root
