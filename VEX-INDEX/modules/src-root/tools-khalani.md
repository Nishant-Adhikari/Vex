...
handlers (`chain_read`, `wallet_balances`, `wallet_resolve`).
...
| `src/vex-agent/tools/internal/chain-read.ts` | `client`, `chains`, `evm-client` | `chain_read` tool: chain lookup + public client for on-chain forensics (tx receipts, ERC-721 mint detection) |
...
- Z3 `module.vex-agent.tools-internal` — `wallet_balances` (balances), `chain_read` (public client + chain lookup), `wallet/resolve.ts` (ChainFamily type), internal khalani alias shim
...
- related internal tools: `module.vex-agent.tools-internal` (khalani internal alias shim, wallet_balances, chain_read)
