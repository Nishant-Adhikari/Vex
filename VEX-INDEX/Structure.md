...
- F4 fixed: `OpenRouterProvider.loadConfig()` now memoizes the last-good `InferenceConfig` on the instance — fresh hits within `MODEL_CONFIG_CACHE_TTL_MS` (1h) reuse it (shallow copy), concurrent callers dedup on one in-flight fetch, and a transient `/models` failure with a last-good serves stale (throttled by `MODEL_CONFIG_STALE_RETRY_MS`, 30s) instead of killing the turn. A successful catalog that no longer lists the model still returns null (stays loud); first fetch with no last-good still returns null. Raw `/models` read extracted to private `_fetchConfig()`; the 6 per-turn call sites and `loadConfig()` signature unchanged. Compaction chunker's per-job `new OpenRouterProvider()` is out of scope (per-job, not per-turn).
...
| **F4** | OpenRouter `loadConfig()` fixed: memoizes last-good `InferenceConfig` on the instance — TTL reuse (1h), in-flight dedup with per-caller copy, stale-serve on transient `/models` failure (30s re-attempt throttle) instead of a dead turn; `model_not_found` and cold first-fetch still return null. The "provider reset not called on generic vault unlock" clause is moot: a locked-boot `null` provider is never cached so the next post-unlock resolve builds fresh, and lock already calls `resetProvider()` (Bundle A security-003). | fixed | MED | `src/vex-agent/inference/openrouter.ts:98` (loadConfig cache + `_fetchConfig`), `src/vex-agent/inference/config.ts` (`MODEL_CONFIG_CACHE_TTL_MS`/`MODEL_CONFIG_STALE_RETRY_MS`), `registry.ts:134`, commit `e02f73b` |
