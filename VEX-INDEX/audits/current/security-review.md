...
| FINDING-security-004 | Wallet keystore KDF parity + OWASP scrypt cost | fixed | Tracked from Z5. Bundle A (B1a) first raised keystore to vault parity (65536); Bundle B (F10-OWASP, commit 1c858ee) then raised BOTH at-rest scrypt KDFs to OWASP Password Storage recommendation N=131072 (2^17), r=8, p=1, dkLen=32 in lockstep — keystore-vs-vault asymmetry resolved. `src/tools/wallet/keystore.ts:25 KDF_PARAMS` (2**17) and `src/lib/local-secret-vault.ts:31 CURRENT_KDF_PARAMS.N` (131072) now equal. Both stores persist per-file KDF params (vault opportunistically rewrites to CURRENT on successful unlock; keystore decrypt uses the file's own params), so older files still decrypt. `deriveKey` maxmem=256 MiB; 2^17 needs ~128 MiB; 2^18 rejected at that ceiling (ERR_CRYPTO_INVALID_SCRYPT_PARAMS). No migration (pre-release dev); ~400ms/derive. Codex-gated (session f10-owasp-harness, GREEN); 102 keystore/vault/wallet crypto tests pass. |
...
| FINDING-codex-001 | Wallet auto-backup omitted newer `wallet-<id>.json` inventory keystores (+ vault) | fixed | Bundle B / B1b (commits a35d4f4 C1, 6a1f7ab C2, 53f1266 C3). `autoBackup` now captures the FULL wallet surface (legacy + per-id keystores + `secrets.vault.json` + `.env` + `config.json`, manifest v2) and a new symmetric archive-restore primitive (`src/tools/wallet/backup-restore.ts`) + IPC (`walletListBackups`/`walletRestoreArchive`) + renderer screen recover it. Fail-closed validation (realpath/symlink/manifest 1:1/dup-address/signer-mismatch), staged journaled commit with rollback, mandatory pre-restore snapshot, `.env` sanitize, role-based vault refresh. ADD-flow hooks back up new wallets on create/import. Codex GREEN LIGHT (plan v3 + per-checkpoint final reviews). |
