...
| 7 | executor calls Track-2 chunker `compact-jobs/chunker-call.ts:63` — directly constructs `OpenRouterProvider` (Round-1 finding: bypasses inference registry singleton/reset path) | OpenRouter chunker generates structured chunks from archived prefix | none | API failure → row marked failed, retry budget exhausts |
