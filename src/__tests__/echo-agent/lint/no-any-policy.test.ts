 ...
 * `as unknown as` regressions in `src/echo-agent/` runtime code.
 *
 ...
 * documented in `src/echo-agent/AUDIT_INVENTORY.md` §4.
 *
 * This test walks every `.ts` under `src/echo-agent/` that is NOT a
...
const ROOT = resolve(process.cwd(), "src/echo-agent");

/** Maximum number of `as any` / `as unknown as` occurrences allowed in runtime code. */
const MAX_ALLOWED = 2;

/** Directories inside `src/echo-agent/` that are NOT counted as runtime code. */
...
describe("no-any policy — runtime code in src/echo-agent/", () => {
  it(`total 'as any' + 'as unknown as' count stays at or below ${MAX_ALLOWED}`, () => {
    ...
    for (const file of files) {
      ...
      if (count > 0) report.push({ path: file.replace(ROOT, "src/echo-agent"), count });
      total += count;
    }
    ...
    if (total > MAX_ALLOWED) {
      ...
      throw new Error(
        `no-any policy violated: ${total} escapes found, budget is ${MAX_ALLOWED}.\n` +
          ...
          `src/echo-agent/AUDIT_INVENTORY.md §4 and bump MAX_ALLOWED with a note.`,
      );
    }
