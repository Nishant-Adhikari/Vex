...
describe("validateZapRouteResponse — equivalence", () => {
  ...
  it("defaults every field on empty/missing data (each default lands)", () => {
    const r = validateZapRouteResponse({ data: {} });
    expect(r.code).toBe(0); // missing code → 0
    expect(r.message).toBeUndefined();
    expect(r.data.routeSummary).toBeUndefined();
    expect(r.data.zapDetails).toBeUndefined();
    expect(r.data.route).toBeUndefined();
    expect(r.data.routerAddress).toBeUndefined();
  ...
  it("non-record data collapses to {} → all data fields default", () => {
    const r = validateZapRouteResponse({ code: 1, data: "not-a-record" });
  ...
  it("code accepts ±Infinity (proves NOT z.number())", () => {
    expect(validateZapRouteResponse({ code: Infinity, data: {} }).code).toBe(Infinity);
    expect(validateZapRouteResponse({ code: -Infinity, data: {} }).code).toBe(-Infinity);
  });

  it("code accepts NaN (proves NOT zNumberField — original had no NaN check)", () => {
    expect(Number.isNaN(validateZapRouteResponse({ code: NaN, data: {} }).code)).toBe(true);
  });

  it("non-number code (string) falls back to 0", () => {
    expect(validateZapRouteResponse({ code: "5", data: {} }).code).toBe(0);
  });

  it("empty-string route/gas/gasUsd PASS (bare typeof string, not asOptionalString)", () => {
    const r = validateZapRouteResponse({ data: { route: "", gas: "", gasUsd: "" } });
  ...
  it("empty-string message/requestId become undefined (asOptionalString drops '')", () => {
    const r = validateZapRouteResponse({ message: "", requestId: "", data: {} });
    expect(r.message).toBeUndefined();
    expect(r.requestId).toBeUndefined();
  });

  it("routeSummary preserves any non-nullish value; null/undefined → undefined", () => {
    expect(validateZapRouteResponse({ data: { routeSummary: 0 } }).data.routeSummary).toBe(0);
    expect(validateZapRouteResponse({ data: { routeSummary: [1, 2] } }).data.routeSummary).toEqual([1, 2]);
    expect(validateZapRouteResponse({ data: { routeSummary: null } }).data.routeSummary).toBeUndefined();
    expect(validateZapRouteResponse({ data: { routeSummary: "s" } }).data.routeSummary).toBe("s");
  });

  it("zapDetails preserved only when record, else undefined", () => {
    expect(validateZapRouteResponse({ data: { zapDetails: { a: 1 } } }).data.zapDetails).toEqual({ a: 1 });
    expect(validateZapRouteResponse({ data: { zapDetails: "x" } }).data.zapDetails).toBeUndefined();
    expect(validateZapRouteResponse({ data: { zapDetails: [1] } }).data.zapDetails).toBeUndefined();
  });

  it("poolDetails: built record but per-field type mismatches → undefined fields", () => {
    const r = validateZapRouteResponse({
      data: { poolDetails: { category: 123, token0: "0xa", fee: "nope", address: 5 } },
    });
  ...
  it("positionDetails: number fields accept NaN/Infinity (bare typeof number)", () => {
    const r = validateZapRouteResponse({
      data: { positionDetails: { tokenId: "1", tickLower: NaN, tickUpper: Infinity, liquidity: "9" } },
    });
  ...
  it("non-record poolDetails/positionDetails → whole sub-object undefined", () => {
    const r = validateZapRouteResponse({ data: { poolDetails: "x", positionDetails: [1] } });
    expect(r.data.poolDetails).toBeUndefined();
    expect(r.data.positionDetails).toBeUndefined();
  });

  it("non-string routerAddress → undefined", () => {
    expect(validateZapRouteResponse({ data: { routerAddress: 5 } }).data.routerAddress).toBeUndefined();
  });
});
