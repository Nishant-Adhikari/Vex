...
const { renderConnectorDetails } = await import("../../cli/setup/ui.js");

afterEach(() => {
  vi.clearAllMocks();
});

describe("setup connector UI rendering", () => {
  it("prints action-first shell and AI guidance for connector details", () => {
    renderConnectorDetails(
      {
        ...
        commandPreview: "claude mcp add-json --scope local vex ...",
        nextSteps: ["Run the generated command."],
        quickstartPrompt: "Use the connected Vex MCP in read-only mode first.\nRead docs://overview.",
        artifacts: [
          {
          ...
          },
          {
            ...
            content: "Use the connected Vex MCP in read-only mode first.\n",
    ...
    expect(output).toContain("Use the connected Vex MCP in read-only mode first.");
    expect(output).toContain("Read docs://overview.");
    expect(output).not.toContain("| RUN IN SHELL |");
    expect(output).not.toContain("| PASTE INTO AI |");
    expect(output).not.toContain("| Use the connected Vex MCP in read-only mode first.");
