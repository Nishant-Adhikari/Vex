...
import { ingestSignalsFeed, signalsFeedHeaders } from "@vex-agent/signals/ingest.js";
...
describe("signalsFeedHeaders", () => {
  // TrendRadar's feed repo is private, so the production fetch reads it through
  // the GitHub Contents API and must present a read-only token via `Authorization`
  // plus the `raw` media type (returns the file bytes, not a base64 envelope).
  it("requests the raw media type", () => {
    expect(signalsFeedHeaders(undefined).Accept).toBe("application/vnd.github.raw");
  });

  it("attaches a bearer token when one is provided", () => {
    expect(signalsFeedHeaders("gh_tok_123").Authorization).toBe("Bearer gh_tok_123");
  });

  it("omits Authorization when the token is absent or blank (anonymous fetch)", () => {
    expect(signalsFeedHeaders(undefined).Authorization).toBeUndefined();
    expect(signalsFeedHeaders("   ").Authorization).toBeUndefined();
  });
});
