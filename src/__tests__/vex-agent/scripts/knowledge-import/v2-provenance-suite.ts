...
export function v2ProvenanceSuite(ctx: SuiteCtx): void {
  ...
  describe("v2 provenance roundtrip", () => {
    ...
    it("v2: absent provenance falls through to insertEntry defaults (undefined → 'vex_agent'/NULL)", async () => {
      await importKnowledge(
        lines(
          JSON.stringify({ __type: "vex_knowledge_export", version: 2 }),
          makeRowLine(),
    ...
    it("v2: source_session=null is accepted (maps to undefined → DB NULL)", async () => {
      await importKnowledge(
        lines(
          JSON.stringify({ __type: "vex_knowledge_export", version: 2 }),
          makeRowLine({ source_surface: "vex_agent", source_session: null }),
        ),
      );
      const arg = mockInsertEntry.mock.calls[0]![0];
      expect(arg.sourceSurface).toBe("vex_agent");
      expect(arg.sourceSession).toBeUndefined();
    });
  });
}
