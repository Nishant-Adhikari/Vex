...
vi.mock("@vex-agent/db/repos/checkpoint-handoffs.js", () => ({
...
vi.mock("@vex-agent/db/repos/session-episodes.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@vex-agent/db/repos/session-episodes.js")>();
  return {
    ...actual,
...
vi.mock("@vex-agent/engine/checkpoint/merge.js", () => ({
  summarizePrefix: vi.fn().mockResolvedValue("test summary"),
}));

vi.mock("@vex-agent/engine/checkpoint/extract.js", () => ({
