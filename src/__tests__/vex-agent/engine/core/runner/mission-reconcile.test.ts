...
describe("reconcileOrphanedRuns", () => {
  ...
  it("isolates a per-run failure — the sweep continues and counts it", async () => {
    ...
    // RECONCILE-001: a single closeLedger throw now retries. The mock throws
    // on call #1 (run-1 first attempt), resolves on call #2 (run-1 retry),
    // and resolves on call #3 (run-2). Both runs land as "reconciled".
    // A GENUINE pre-claim failure (claim itself throws) is still "failed" —
    // that invariant is pinned in reconcile-ledger.regression.test.ts.
