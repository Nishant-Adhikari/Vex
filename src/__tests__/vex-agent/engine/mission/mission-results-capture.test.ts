...
describe("computeMissionOpenPositions", () => {
  ...
  it("records only the delta when a mission adds to an existing position", () => {
    expect(computeMissionOpenPositions(
      [{ symbol: "PONS", address: "0xabc", amount: 2, valueUsd: 10 }],
      [{ symbol: "PONS", address: "0xAbC", amount: 5, valueUsd: 40 }],
    )).toEqual([
      { symbol: "PONS", address: "0xAbC", amount: 3, valueUsd: 24 },
    ]);
  });

  it("records a wholly new token position in full", () => {
    expect(computeMissionOpenPositions(
      [],
      [{ symbol: "PONS", address: "0xabc", amount: 5, valueUsd: 40 }],
    )).toEqual([
