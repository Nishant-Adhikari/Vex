...
export const TRENDING_TOOLS: readonly ProtocolToolManifest[] = [
  {
    ...
    lifecycle: "active",
    description: "Get latest trending token profiles — icons, descriptions, social links. Shows what projects are gaining attention. Optional chainId keeps only profiles on that chain (e.g. robinhood) — pass it on a chain-scoped mission so you never surface same-name tokens on other chains.",
    mutating: false,
    actionKind: "read",
    params: [
      { key: "chainId", type: "string", description: "Optional client-side chain filter (e.g. ethereum, base, solana, robinhood). Keeps only profiles on this chain." },
    ],
    exampleParams: { chainId: "robinhood" },
  ...
  },
  {
    ...
    namespace: "dexscreener",
    lifecycle: "active",
    description: "Get latest boosted/promoted tokens with boost amounts. Paid visibility signal — shows where money is being spent on promotion. Optional chainId keeps only boosts on that chain (e.g. robinhood) — pass it on a chain-scoped mission.",
    mutating: false,
    actionKind: "read",
    params: [
      { key: "chainId", type: "string", description: "Optional client-side chain filter (e.g. ethereum, base, solana, robinhood). Keeps only boosts on this chain." },
    ],
    exampleParams: { chainId: "robinhood" },
  ...
  },
  {
    ...
    namespace: "dexscreener",
    lifecycle: "active",
    description: "Get tokens with most active boosts (top promoted). Ranked by total boost amount. Optional chainId keeps only boosts on that chain (e.g. robinhood) — pass it on a chain-scoped mission so `dexscreener.boosts.top chainId=robinhood` returns only that chain's promoted movers.",
    mutating: false,
    actionKind: "read",
    params: [
      { key: "chainId", type: "string", description: "Optional client-side chain filter (e.g. ethereum, base, solana, robinhood). Keeps only boosts on this chain." },
  ...
  },
  {
    ...
    lifecycle: "active",
    description:
      "Synthetic ATTENTION signal — merges token-profiles + paid boosts into one ranked, deduplicated list (boost spend, then profile presence). Shows which specific tokens are buying visibility. This is NOT the official trending feed — use dexscreener.trending for trending narratives. Optional chainId scopes both sources to that chain (e.g. robinhood) BEFORE the merge — pass it on a chain-scoped mission.",
    mutating: false,
    actionKind: "read",
    params: [
      { key: "limit", type: "number", description: "Max results to return (default 20)." },
      { key: "chainId", type: "string", description: "Optional client-side chain filter (e.g. ethereum, base, solana, robinhood). Scopes profiles + boosts to this chain before ranking." },
    ],
    exampleParams: { limit: 20, chainId: "robinhood" },
