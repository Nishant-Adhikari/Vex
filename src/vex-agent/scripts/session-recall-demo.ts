...
import { computeEpisodeHash } from "@vex-agent/engine/checkpoint/extract.js";
...
async function runDemo(): Promise<void> {
  ...
  console.log(`  psql $VEX_DB_URL -c "SELECT id, episode_kind, title, summary_text FROM session_episodes WHERE session_id = '${sessionId}' ORDER BY id;"`);
  console.log("  psql $VEX_DB_URL -c \"SELECT id, memory_scope_key, memory_language_code FROM sessions WHERE id = '" + sessionId + "';\"");
  console.log("Drop the demo session (CASCADE removes episodes):");
  console.log(`  psql $VEX_DB_URL -c "DELETE FROM sessions WHERE id = '${sessionId}';"`);
  console.log("=".repeat(72));
}
