...
import { extractEpisodes, type ExtractionResult } from "@vex-agent/engine/checkpoint/extract.js";
  ...
  async chatCompletionSimple(
    ...
    const res = await fetch(`${this.baseUrl}/chat/completions`, {
      ...
      headers: {
        ...
        "HTTP-Referer": "https://github.com/Vex-Foundation/Vex",
        "X-Title": "Vex checkpoint compliance",
      },
...
- Tighten the directive in \`src/vex-agent/engine/checkpoint/extract.ts\` or \`merge.ts\` — common causes: LLM omits \`title\`, returns the explicit code in a different casing, or adds "Here is the summary:" preamble.
- Re-run this CLI.
`;
}
