...
export interface MissionResultRow {
  ...
  */
  summary: string | null;
  /** Frozen inference config captured on `mission_runs` at run start. */
...
const INFERENCE_STAMP_SUBSELECT = `
  (SELECT r.inference_provider FROM mission_runs r
    WHERE r.id = mission_results.mission_run_id) AS inference_provider,
  (SELECT r.inference_model FROM mission_runs r
    WHERE r.id = mission_results.mission_run_id) AS inference_model,
  (SELECT r.inference_fallback_model FROM mission_runs r
    WHERE r.id = mission_results.mission_run_id) AS inference_fallback_model`;
...
interface Raw {
  ...
  inference_fallback_model?: string | null;
...
function toRow(r: Raw): MissionResultRow {
  return {
    ...
    inferenceProvider: r.inference_provider ?? null,
    inferenceModel: r.inference_model ?? null,
    inferenceFallbackModel: r.inference_fallback_model ?? null,
...
export async function listResultsForWallet(
  ...
  const rows = await query<Raw>(
    `SELECT ${SELECT_COLUMNS}, ${STOP_SUMMARY_SUBSELECT}, ${INFERENCE_STAMP_SUBSELECT}
       FROM mission_results
      WHERE LOWER(wallet_address) = LOWER($1)
...
/** Paper-only mission history, newest first, independent of any live wallet. */
export async function listSimulatedResults(
  limit = 50,
): Promise<MissionResultRow[]> {
  const rows = await query<Raw>(
    `SELECT ${SELECT_COLUMNS}, ${STOP_SUMMARY_SUBSELECT}, ${INFERENCE_STAMP_SUBSELECT}
