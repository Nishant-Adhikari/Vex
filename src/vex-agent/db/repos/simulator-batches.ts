...
export async function listBatchIdsForMissionRuns(
  missionRunIds: readonly string[],
): Promise<ReadonlyMap<string, string>> {
  if (missionRunIds.length === 0) return new Map();
  const rows = await query<Record<string, unknown>>(
    `SELECT mission_run_id, batch_id
       FROM simulator_batch_entries
      WHERE mission_run_id = ANY($1::text[])`,
    [missionRunIds],
  );
  const byRunId = new Map<string, string>();
  for (const row of rows) {
    const missionRunId = row.mission_run_id as string | undefined;
    const batchId = row.batch_id as string | undefined;
    if (!missionRunId || !batchId || byRunId.has(missionRunId)) continue;
    byRunId.set(missionRunId, batchId);
  }
  return byRunId;
}
