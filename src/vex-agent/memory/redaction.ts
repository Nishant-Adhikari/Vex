 ...
 *     - BIP39 mnemonic phrases (12/15/18/21/24-word sequences)
 *     - Private keys (high-entropy hex / base58 of crypto-key length)
 *     - Bearer tokens / API keys (well-known prefixes)
 *     - JWT
 *
 * Tier 2 — MASK
 *   Identifier shape preserved (`0xabcd…1234`) so the agent still
 *   understands the semantic role of the value. Reversible by querying
 *   structured tables (`proj_*`, on-chain reads).
 ...
 * the caller can decide whether the chunk retains enough signal to embed.
 */
