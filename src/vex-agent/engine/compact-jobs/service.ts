...
import { withCheckpointMutex } from "@vex-agent/engine/core/checkpoint/state.js";
...
async function executeCompactNowInner(input: CompactCommitArgs): Promise<CompactCommitResult> {
  ...
  const messagesWithId = await messagesRepo.getLiveMessagesWithId(input.sessionId);
