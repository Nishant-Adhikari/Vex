...
export async function runConnectFlow(): Promise<void> {
  ...
  renderSection(
    "Connect Vex to your AI agent",
    "This flow sets up the local Vex MCP, configures both wallets, starts local services, and writes ready connectors for Cursor, Claude Code, Codex, OpenClaw, and a default MCP client.",
  );
...
export async function runLauncherMenu(): Promise<void> {
  ...
  const selection = await promptMenu("Vex", [
    {
      id: "connect",
      label: "Connect Vex to your AI agent",
      description:
        "Set up the local Vex MCP, configure wallets, start services, and generate ready connectors for Cursor, Claude Code, Codex, OpenClaw, and a default MCP client.",
    },
  ]);
