...
export function assertInteractiveLauncher(): void {
  if (isHeadless() || stdin.isTTY !== true) {
    throw new VexError(
      ...
      "vex setup requires an interactive terminal.",
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    const onData = (chunk: Buffer | string): void => {
      ...
      for (const char of text) {
        if (char === "\u0003") {
          cleanup();
          reject(new VexError(ErrorCodes.SETUP_CANCELLED, "Setup cancelled by user."));
          return;
        }
...
export function renderLauncherHeader(coreReady: boolean): void {
  ...
  if (coreReady) {
    ...
    writeStderr(color("Setting up your local Vex MCP", ANSI.cyan));
  }
}
