...
function printManualDockerInstructions(): never {
  ...
  renderSection(
    "Docker required",
    "EchoClaw needs Docker, Docker Compose, and Docker Model Runner before the local MCP can start.",
  );
  ...
  if (fallback) {
    ...
    writeStderr("- Install or enable Docker Model Runner before rerunning `echoclaw echo`.");
  }

  throw new EchoError(
    ErrorCodes.SYSTEM_CHECK_FAILED,
    "Docker is not installed.",
    "Install Docker and rerun `echoclaw echo`.",
  );
}
...
export async function maybeInstallDockerWhenMissing(dockerDetail: string): Promise<never> {
  ...
  const shouldInstall = await confirm(
    `Docker is not installed. Do you want EchoClaw to install Docker Engine for ${distro} now?`,
    true,
  );
  ...
  throw new EchoError(
    ErrorCodes.SYSTEM_CHECK_FAILED,
    "Docker was installed successfully, but the current shell session must be refreshed before EchoClaw can use it.",
    addedToGroup
      ? "Sign out and back in, or start a fresh terminal session, then rerun `echoclaw echo`."
      : "Start a fresh terminal session, then rerun `echoclaw echo`.",
  );
}
...
export async function maybeInstallDockerModelPluginWhenMissing(modelRunnerDetail: string): Promise<boolean> {
  ...
  const shouldInstall = await confirm(
    "Docker Model Runner plugin is missing. Do you want EchoClaw to install it now?",
    true,
  );
  ...
  if (!installRunner.ok) {
    writeStderr("Docker Model Runner plugin was installed. EchoClaw will re-run the system checks next.");
  }
