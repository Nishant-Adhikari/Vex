 ...
 *
 * Re-validates every emit through `transcriptAppendEventSchema` before
 * delivering it to the renderer callback (third validation layer after
 * the engine type-check and the main-side bridge). A misbehaving main
 * never injects unexpected shapes into renderer state.
 */
