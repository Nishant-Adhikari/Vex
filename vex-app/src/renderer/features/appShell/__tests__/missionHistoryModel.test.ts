...
describe("strategy tags", () => {
  it("extracts a strategy tag from the goal snippet header", () => {
    expect(
      extractStrategyTag("LIVE mission\nSTRATEGY TAG: baseline v1.0\nGOAL: ..."),
    ).toBe("baseline v1.0");
  });

  it("returns null when no strategy tag exists", () => {
    expect(extractStrategyTag("GOAL: plain mission without tag")).toBeNull();
    expect(extractStrategyTag(null)).toBeNull();
  });

  it("summarizes runs by strategy tag with win rate and avg pnl", () => {
    const stats = summarizeByStrategy([
      result({
        missionRunId: "a",
        goalSnippet: "LIVE\nSTRATEGY TAG: baseline v1.0\nGOAL: ...",
        pnlEth: 0.002,
  ...
  it("computes deadline flatten rate only from deadline-reached runs", () => {
    const stats = summarizeByStrategy([
      result({
        missionRunId: "a",
        goalSnippet: "LIVE\nSTRATEGY TAG: ghost v1.2\nGOAL: ...",
        outcome: "timed_out",
    ...
    expect(stats[0]?.tag).toBe("ghost v1.2");
    expect(stats[0]?.runs).toBe(3);
    expect(stats[0]?.winRate).toBeCloseTo(66.6666666667, 6);
    expect(stats[0]?.avgPnlEth).toBeCloseTo(0.0006666666666666666, 12);
    expect(stats[0]?.flatCloseRate).toBeCloseTo(66.6666666667, 6);
    expect(stats[0]?.deadlineFlattenRate).toBe(50);
  });
});
