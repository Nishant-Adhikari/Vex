...
describe("MissionControls — paused_error standing alert (issue #42)", () => {
  ...
  it("surfaces the persisted stop summary when present", async () => {
    getStateMock.mockResolvedValue(
      runtimeState({
        hasActiveRun: true,
        status: "paused_error",
    ...
    const alert = await screen.findByRole("alert");
    expect(alert.textContent).toMatch(/OpenRouter chat completion failed: status=403/);
    expect(alert.textContent).toMatch(/Budget limit exceeded \(monthly limit\)/);
  });

  it("upgrades known 401/403 provider failures into a provider-blocked state", async () => {
    getStateMock.mockResolvedValue(
      runtimeState({
        hasActiveRun: true,
        status: "paused_error",
    ...
    const alert = await screen.findByRole("alert");
    expect(alert.getAttribute("data-vex-state")).toBe("provider-blocked");
    expect(alert.textContent).toMatch(/Provider blocked/);
    expect(alert.textContent).toMatch(
