...
import { STRATEGY_TAG_PREFIX } from "../../../shared/simulator/pons-paper-strategies.js";

export const EM_DASH = "—";
export type MissionHistoryFilter = "all" | "live" | "paper";
export interface StrategyStats {
  readonly tag: string;
  readonly runs: number;
  readonly winRate: number | null;
  readonly avgPnlEth: number | null;
  readonly flatCloseRate: number | null;
  readonly deadlineFlattenRate: number | null;
}
...
export function filterMissionResults(
  results: readonly MissionResultDto[],
  filter: MissionHistoryFilter,
): readonly MissionResultDto[] {
  switch (filter) {
    case "live":
      return results.filter((r) => !r.simulated);
    case "paper":
      return results.filter((r) => r.simulated);
    default:
      return results;
...
export function deriveMissionHistoryTitle(
  result: Pick<MissionResultDto, "seqNo" | "simulated">,
  sessionTitle: string | null | undefined,
): string {
  const title = sessionTitle?.trim();
  if (title) return title;
  return result.simulated ? `Paper Mission #${result.seqNo}` : `Mission #${result.seqNo}`;
}

export function extractStrategyTag(
  goalSnippet: string | null | undefined,
): string | null {
  const value = goalSnippet?.trim();
  if (!value) return null;
  const re = new RegExp(`${STRATEGY_TAG_PREFIX.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*([^\\n]+)`, "i");
...
export function summarizeByStrategy(
  ...
  for (const result of results) {
    const tag = extractStrategyTag(result.goalSnippet);
    if (!tag) continue;
  ...
  return [...grouped.entries()]
    .map(([tag, rows]) => {
      ...
      const flatRows = rows.filter((row) => row.openPositionsCount === 0);
      const deadlineRows = rows.filter((row) => row.stopReason === "deadline_reached");
      const deadlineFlatRows = deadlineRows.filter((row) => row.openPositionsCount === 0);
      const avgPnlEth =
        knownPnls.length > 0
      ...
      return {
        ...
        avgPnlEth,
        flatCloseRate: rows.length > 0 ? (flatRows.length / rows.length) * 100 : null,
        deadlineFlattenRate:
          deadlineRows.length > 0 ? (deadlineFlatRows.length / deadlineRows.length) * 100 : null,
      };
    })
