...
function fmtBatchTimestamp(iso: string): string {
...
export function SimulatorPanel(): JSX.Element {
  ...
  const batchLabelById = useMemo(() => {
    const items =
      batchHistoryQuery.data && batchHistoryQuery.data.ok
        ? batchHistoryQuery.data.data
        : [];
    const byId = new Map<string, string>();
    for (const [index, batch] of items.entries()) {
      byId.set(batch.id, `Batch #${items.length - index}`);
    }
    return byId;
  }, [batchHistoryQuery.data]);
...
function PromptsView({
  ...
  const diffSummary = summarizePromptVersionDiffs(leftPromptVersion, rightPromptVersion);
  return (
      ...
      {promptVersionOptions.length >= 1 ? (
        <div className="flex flex-col gap-4">
          <div className="rounded-xl border border-[var(--vex-line)] bg-white/[0.02] p-3">
            ...
            <div className="text-[11px] text-[var(--vex-text-3)]">
              {leftPromptVersion?.version ?? "—"} → {rightPromptVersion?.version ?? "—"}
            </div>

            <ul className="mt-3 list-disc space-y-1 pl-5 text-[12px] leading-relaxed text-[var(--vex-text-2)]">
              {diffSummary.map((bullet) => (
...
function summarizeStrategyPerformance(results: readonly MissionResultDto[]): readonly {
  ...
  const derivePnlPct = (row: MissionResultDto): number | null => {
    if (row.pnlPct !== null && Number.isFinite(row.pnlPct)) return row.pnlPct;
    if (
      row.pnlEth !== null &&
      Number.isFinite(row.pnlEth) &&
      row.bankrollStartEth !== null &&
      Number.isFinite(row.bankrollStartEth) &&
      row.bankrollStartEth > 0
    ) {
      return (row.pnlEth / row.bankrollStartEth) * 100;
    }
    return null;
  };
  ...
  return [...grouped.entries()]
    .map(([tag, rows]) => {
      ...
      return {
        tag,
        count: rows.length,
        winRate: winRate === null ? "—" : `${Math.round(winRate)}%`,
        avgPnlPct: avg,
      };
    })
...
function normalizePromptDiffValue(value: string): string {
  return value.replace(/\s+/g, " ").trim();
}

function formatPromptDiffPreview(value: string): string {
  const normalized = normalizePromptDiffValue(value);
  if (normalized.length <= 140) return normalized;
  return `${normalized.slice(0, 137)}...`;
}

function parsePromptSections(promptText: string): Map<string, string> {
  ...
  for (const rawLine of promptText.split("\n")) {
    ...
    sections.set(`Line ${unlabeledIndex}`, line);
    unlabeledIndex += 1;
  }
  return sections;
}

function summarizePromptVersionDiffs(
  leftPromptVersion: {
  ...
  if (leftPromptVersion === null || rightPromptVersion === null) return [];
  if (leftPromptVersion.version === rightPromptVersion.version) {
    return ["Both sides are the same prompt version."];
  }

  const leftSections = parsePromptSections(leftPromptVersion.promptText);
  const rightSections = parsePromptSections(rightPromptVersion.promptText);
  const sectionKeys = new Set([...leftSections.keys(), ...rightSections.keys()]);
  const bullets: string[] = [];
  const seen = new Set<string>();

  for (const key of sectionKeys) {
    const leftValue = leftSections.get(key);
    const rightValue = rightSections.get(key);
    if (leftValue !== undefined && rightValue !== undefined) {
      if (normalizePromptDiffValue(leftValue) === normalizePromptDiffValue(rightValue)) continue;
      const bullet = `${key}: changed from “${formatPromptDiffPreview(leftValue)}” to “${formatPromptDiffPreview(rightValue)}”.`;
      if (!seen.has(bullet)) {
        bullets.push(bullet);
        seen.add(bullet);
      }
    ...
    if (leftValue !== undefined) {
      const bullet = `${key}: removed “${formatPromptDiffPreview(leftValue)}”.`;
      if (!seen.has(bullet)) {
        bullets.push(bullet);
        seen.add(bullet);
      }
    ...
    if (rightValue !== undefined) {
      const bullet = `${key}: added “${formatPromptDiffPreview(rightValue)}”.`;
      if (!seen.has(bullet)) {
        bullets.push(bullet);
        seen.add(bullet);
      }
  ...
  if (bullets.length === 0) {
    return ["No rule-level changes detected. The wording differences are cosmetic only."];
  }
  ...
  return (
    <section className="border-b border-[var(--vex-line)] pb-6">
      ...
      <div className="grid gap-2">
        {query.data.data.map((batch, index) => (
          <BatchHistoryRow
            key={batch.id}
            batch={batch}
            batchNumber={query.data.data.length - index}
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-[8px] border border-[var(--vex-line)] px-3 py-2">
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <div className="truncate font-mono text-[10px] uppercase tracking-[0.08em] text-[var(--vex-text-3)]">
            {batch.id}
          </div>
          {current ? (
            <span className="rounded-[4px] border border-[var(--vex-accent)]/40 px-1 py-0.5 font-mono text-[9px] uppercase tracking-[0.08em] text-[var(--vex-accent)]">
      ...
      <div className="text-right font-mono text-[11px] text-[var(--vex-text-3)]">
        <div>{fmtBatchTimestamp(batch.createdAt)}</div>
...
function PaperMissionLedger({
  ...
  readonly batchLabelById: ReadonlyMap<string, string>;
...
function PaperMissionRow({
  ...
  const batchLabel =
    result.sourceBatchId === null || result.sourceBatchId === undefined
      ? "—"
      : (batchLabelById.get(result.sourceBatchId) ?? result.sourceBatchId);
      ...
      <td className="py-2.5 pr-3">
        <span
          className={cn(
            ...
            batchLabel === "—" ? "text-[var(--vex-text-3)]" : "text-[var(--vex-text-2)]",
          )}
          title={result.sourceBatchId ?? undefined}
        >
          {batchLabel}
        </span>
      </td>
