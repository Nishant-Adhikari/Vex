...
| Solana / Jupiter | `references/solana/solana-jupiter.md` | Solana DeFi via Jupiter (requires JUPITER_API_KEY): Swap API V2 (aggregates Raydium, Orca, Meteora), token browse (trending/top-traded/top-organic/recent/lst/verified), price lookup (Jupiter Price V3), lending via Jupiter Lend Earn (rates/positions/deposit/withdraw/earnings), prediction markets (YES/NO binary contracts — list/search/event/market/buy/sell/claim/close-all/positions/history). SOL/SPL transfers (2-step prepare→confirm), SPL token burn and empty account closure. |
