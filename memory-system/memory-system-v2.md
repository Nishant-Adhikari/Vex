...
### S6 — Maturity FSM + activation + decay (+graceful degradation) `[~] S6a DONE; S6b TODO` (2026-06-09; Codex plan-gate GREEN po 2 rundach + Phase-6 impl-gate GREEN po 1 fixie; sesja harness-memory-s6; spec: `memory-system/s6-plan.md`)
...
> **S6b TODO** (osobny etap): dzienny regime worker (Tavily/Twitter conditional) + `regime_snapshots` + regime-aware decay/reaktywacja. Design w `s6-plan.md §12`. Nie replay-stabilny (świadome) + poisoning-wrażliwy → izolowany.
